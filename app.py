...
@app.route("/home")
def home():
    return render_template('views/home.html', user=USER, pages=get_pages(), app_name=APP_NAME, envvar=ENV_VAR)
...
def config():
    ...
    return render_template('views/config.html', host=host, port=port, datacenter=datacenter, pages=get_pages(), app_name=APP_NAME, envvar=ENV_VAR)
